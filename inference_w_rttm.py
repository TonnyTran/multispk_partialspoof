...
import numpy as np
from partial_spoof_detection.models.conformer import FrameConformer 
from partial_spoof_detection.libs.tool import *
import os
from partial_spoof_detection.libs.startup_config import set_random_seed
import warnings
...
def print_spoof_timestamps(start_time, pred, rso=20):
    start = None
    ...
    for i, value in enumerate(pred):
        if value == 0:
            ...
            if start is not None:
                ...
                txt_output.append(f"{start_time+start}ms - {start_time+end}ms")
                start = None
...
def Inference_mod(file_path, rttm_file, conformer_model, score_type_ouput, consolidate_output, device, base_rso, output_rso):
    print("++++++++++++++++++inference++++++++++++++++++")
    ...
    with torch.no_grad():
        ...
        for line in lines:
            ...
            start_time = float(parts[3]) * sample_rate
            end_time = (start_time + float(parts[4])) * sample_rate
            chunk = wave[:, start_time:end_time]
            ...
            audio_duration = float(parts[4])
            text_list.append("Duration of audio segment: {} from {} to {} s".format(audio_duration, start_time, end_time))
            text_list.append("Percentage Spoof in segment: {}%".format(round((1 - sum(pred) / len(pred)) * 100, 2)))

            if sum(pred) != len(pred):
                text_list.append("Spoof audio segments:")
                seg_text = print_spoof_timestamps(start_time, pred)
                text_list += seg_text
...
if __name__ == '__main__':
    ...
    ###########INFERENCE#############
    output_text = Inference_mod(args.data_path, args.rttm_path, conformer_model, args.score_type_ouput, args.consolidate_output, device, args.base_rso, args.output_rso)
